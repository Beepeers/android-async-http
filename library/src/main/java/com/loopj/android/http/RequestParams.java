...
public class RequestParams implements Serializable {
    ...
    protected boolean chunkedRequest = false;
    ...
    public void setChunkedRequest(boolean chunked) {
        this.chunkedRequest = chunked;
    }
    ...
    public HttpEntity getEntity(ResponseHandlerInterface progressHandler) throws IOException {
        if (useJsonStreamer) {
            ...
            return createChunkedEntity(progressHandler);
    ...
    private HttpEntity createChunkedEntity(ResponseHandlerInterface progressHandler) throws IOException {
        SimpleChunkedEntity entity = new SimpleChunkedEntity(progressHandler);

        if (fileParams.size() > 1) {
            throw new IllegalArgumentException("Chunked streams supports only one file");
        }

        for (ConcurrentHashMap.Entry<String, FileWrapper> entry : fileParams.entrySet()) {
            entity.setFile(entry.getKey(), entry.getValue());
        }
